...
PICKUP_SITE_FIELDS = {"configured", "x_mm", "y_mm", "yaw_ddeg"}
DROP_SITE_FIELDS = {"configured", "x_mm", "y_mm"}
...
def validate_project_sites(data: object, object_type: str, field_path: str) -> dict[str, Any]:
    ...
    if actual_keys != expected_keys:
        raise V40ValidationError(
            object_type,
            field_path,
            "sites must contain exactly the ten fixed V4.0 site keys",
    ...
    for key in REQUIRED_SITE_KEYS:
        ...
        fields = PICKUP_SITE_FIELDS if key in PICKUP_SITE_KEYS else DROP_SITE_FIELDS
        reject_unknown_fields(raw, fields, object_type, f"{field_path}.{key}")
        require_fields(raw, fields, object_type, f"{field_path}.{key}")

...
from hjmb_pathgen.models.enums import ActionCode, ActionMode, FinishMode, NodeFlag, RouteFamily, SegmentFlag
from hjmb_pathgen.models.errors import BinaryLayoutError
...
def _validate_actions(trajectory: CompiledTrajectoryV40) -> None:
    ...
    if header.route_family == int(RouteFamily.MANUAL_FREE):
        return
    if not trajectory.actions:
        _fail("actions", "formal TASK_COMPILED output requires a final DROP STOP_AND_WAIT action")

...
from hjmb_pathgen.codec.csv_codec import load_traj_id_csv
from hjmb_pathgen.codec.json_codec import load_case, load_leg_library, load_project
from hjmb_pathgen.services.collision_config_service import validate_collision_config
...
from hjmb_pathgen.services.manual_path_service import plan_manual_case, retime_case
from hjmb_pathgen.services.mode_output_service import export_final_bin, write_manual_free_outputs
...
from hjmb_pathgen.services.leg_library_service import show_leg, upsert_leg
...
from hjmb_pathgen.services.path_validation_service import validate_case_collision, validate_leg_collision
from hjmb_pathgen.services.phase7_generation_service import (
    audit_phase6,
    collect_unique_legs,
...
from hjmb_pathgen.services.plan_lock_service import list_candidates, lock_candidate, unlock_candidate
from hjmb_pathgen.services.project_config_service import validate_project_site_configuration
from hjmb_pathgen.services.project_service import ProjectLayout
from hjmb_pathgen.services.route_assembler import transition_requirements_from_case
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m hjmb_pathgen.cli")
    ...
    generate_one_parser.add_argument("--replace-manual", action="store_true")
    ...
    write_manual_parser = _project_parser(subparsers.add_parser("write-manual-free"))
    ...
    export_final_parser.add_argument("--source", choices=[item.value for item in PathSource], required=True)
...
def _dispatch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.command == "generate-one":
        return generate_one(
            ...
            replace_manual=args.replace_manual,
        ).to_dict()
    if args.command == "generate-all":
        return generate_all(layout, write_portable=args.portable, dry_run=args.dry_run).to_dict()
    ...
    if args.command == "write-manual-free":
        case = load_case(args.case, enforce_filename=False)
        return write_manual_free_outputs(layout, case, profile_name=args.profile, dry_run=args.dry_run).to_dict()
    if args.command == "export-final":
        return export_final_bin(
            layout,
            args.traj_id,
            path_source=PathSource(args.source),

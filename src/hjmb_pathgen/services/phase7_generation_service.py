...
from hjmb_pathgen.codec.bin_codec import decode_trajectory, encode_trajectory
from hjmb_pathgen.codec.json_codec import load_case, load_leg_library, load_project, load_route_case_table
from hjmb_pathgen.models.compiled import CompiledTrajectoryV40
from hjmb_pathgen.models.enums import ActionMode, LegState, NodeFlag, PathSource, YawPolicy
from hjmb_pathgen.models.errors import CompileError, WriteBackValidationError
...
from hjmb_pathgen.planning.leg_optimizer import leg_id_from_key, leg_key_from_request
...
from hjmb_pathgen.services.leg_library_service import load_or_create_leg_library, save_leg_library_checked, upsert_leg
from hjmb_pathgen.services.export_guard_service import check_formal_export_guard
from hjmb_pathgen.services.leg_optimization_service import leg_request_from_transition, optimize_transition_leg, validate_leg
from hjmb_pathgen.services.leg_stale_service import leg_stale_reasons
from hjmb_pathgen.services.output_service import CaseOutputOptions, CaseOutputResult, write_case_outputs
...
def optimize_missing_legs(
    ...
    *,
    ...
    return OptimizeMissingLegsResult(
        ...
        skipped_count=len(collection.requirements) - len(targets),
...
def generate_one(
    ...
    *,
    ...
    row = _row_by_traj_id(table, traj_id)
    _ensure_no_legacy_manual_overwrite(layout, traj_id, replace_manual=replace_manual)
    ...
    output = write_case_outputs(
        ...
        CaseOutputOptions(
            ...
            use_mode_layout=True,
            path_source=PathSource.TASK_COMPILED,
        ),
    )
...
def validate_one(
    ...
    *,
    ...
    project = load_project(layout.project_json)
    library = load_leg_library(layout.leg_library_json)
    case_path = _existing_case_path(layout, traj_id, PathSource.TASK_COMPILED)
...
def _load_existing_task_case(layout: ProjectLayout, traj_id: int) -> CaseManifestV40 | None:
    for path in (layout.case_json_path_for_source(traj_id, PathSource.TASK_COMPILED), layout.case_json_path(traj_id)):
        if not path.exists():
            continue
        case = load_case(path)
        if case.path_source == PathSource.TASK_COMPILED:
            return case
    return None


def _existing_case_path(layout: ProjectLayout, traj_id: int, path_source: PathSource) -> Path:
    mode_path = layout.case_json_path_for_source(traj_id, path_source)
    if mode_path.exists():
        return mode_path
    legacy_path = layout.case_json_path(traj_id)
    if legacy_path.exists():
        return legacy_path
    raise CompileError(f"P{traj_id:04d} case JSON not found for {path_source.value}")


def _ensure_no_legacy_manual_overwrite(layout: ProjectLayout, traj_id: int, *, replace_manual: bool) -> None:
    legacy_path = layout.case_json_path(traj_id)
    if not legacy_path.exists():
        return
    case = load_case(legacy_path)
    if case.path_source == PathSource.MANUAL_FREE and not replace_manual:
        raise CompileError(
            f"P{traj_id:04d} legacy flat MANUAL_FREE case exists at {legacy_path}; "
            "use --replace-manual only when intentionally replacing or superseding that manual output"
        )

...
from hjmb_pathgen.services.collision_config_service import compute_collision_hashes, validate_collision_config
...
def validate_project_site_configuration(project: ProjectV40) -> ProjectConfigurationReport:
    ...
    ready_manual = not missing_sites and not missing_profiles and _vehicle_complete(project.vehicle) and _dynamics_complete(project.dynamics)
    ready_full = ready_manual and bool(project.action_profiles) and bool(project.topology_profiles)
    if ready_full:

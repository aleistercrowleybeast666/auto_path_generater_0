...
from hjmb_pathgen.codec.bin_codec import decode_trajectory, encode_trajectory, load_bin
...
from hjmb_pathgen.codec.json_codec import load_case, load_project, load_route_case_table
from hjmb_pathgen.models.enums import ActionCode, ActionMode, FinishMode, NodeFlag, PathSource, SegmentFlag
...
def output_layout_report(layout: ProjectLayout) -> dict[str, Any]:
    ...
    expected_dirs = {
        "cases/task_compiled": layout.task_compiled_cases_dir,
        "cases/manual_free": layout.manual_free_cases_dir,
        "bin/task_compiled": layout.task_compiled_bin_dir,
        "bin/manual_free": layout.manual_free_bin_dir,
        "bin/final": layout.final_bin_dir,
        "portable/task_compiled": layout.task_compiled_portable_dir,
        "portable/manual_free": layout.manual_free_portable_dir,
    }
    mismatches: list[dict[str, Any]] = []
    mismatches.extend(_case_dir_mismatches(layout.task_compiled_cases_dir, PathSource.TASK_COMPILED))
    mismatches.extend(_case_dir_mismatches(layout.manual_free_cases_dir, PathSource.MANUAL_FREE))
    return {
        ...
        "path_source_mismatch_count": len(mismatches),
        "path_source_mismatches": mismatches,
...
def generate_golden_manifest(layout: ProjectLayout, *, include_final: bool = True) -> dict[str, Any]:
    ...
    entries = []
    for source in (PathSource.TASK_COMPILED, PathSource.MANUAL_FREE):
        case_dir = layout.case_json_path_for_source(0, source).parent
        bin_dir = layout.bin_path_for_source(0, source).parent
...
def _case_dir_mismatches(case_dir: Path, expected_source: PathSource) -> list[dict[str, Any]]:
    mismatches: list[dict[str, Any]] = []
    for path in sorted(case_dir.glob("P*.json")):
        try:
            ...
            continue
        if case.path_source != expected_source:
            mismatches.append({"path": str(path), "actual": case.path_source.value, "expected": expected_source.value})
    return mismatches


def _case_manifest_entry(case_path: Path, bin_path: Path, source: PathSource, project_hash: str, table_hash: str) -> dict[str, Any]:
    case = load_case(case_path)
    entry = {
        "traj_id": case.traj_id,
        "path_source": source.value,

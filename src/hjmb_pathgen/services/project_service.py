...
from hjmb_pathgen.models.enums import LegState, PathSource
from hjmb_pathgen.models.errors import ProjectLayoutError
from hjmb_pathgen.models.leg import LegLibraryV40
from hjmb_pathgen.models.project import ProjectV40
from hjmb_pathgen.models.protocol import (
    DIR_BIN,
    DIR_CACHE,
    DIR_CASES,
    DIR_DRAFTS,
...
from .path_naming import bin_name, case_json_name, portable_name, validate_traj_id
...
class ProjectLayout:
    ...
    @property
    def task_compiled_cases_dir(self) -> Path:
        return self.resolve_project_path(Path(DIR_CASES) / DIR_TASK_COMPILED)

    @property
    def manual_free_cases_dir(self) -> Path:
        return self.resolve_project_path(Path(DIR_CASES) / DIR_MANUAL_FREE)
    ...
    @property
    def task_compiled_bin_dir(self) -> Path:
        return self.resolve_project_path(Path(DIR_BIN) / DIR_TASK_COMPILED)

    @property
    def manual_free_bin_dir(self) -> Path:
        return self.resolve_project_path(Path(DIR_BIN) / DIR_MANUAL_FREE)
    ...
    @property
    def task_compiled_portable_dir(self) -> Path:
        return self.resolve_project_path(Path(DIR_PORTABLE) / DIR_TASK_COMPILED)

    @property
    def manual_free_portable_dir(self) -> Path:
        return self.resolve_project_path(Path(DIR_PORTABLE) / DIR_MANUAL_FREE)
    ...
    def case_json_path(self, traj_id: int) -> Path:
        return self.resolve_project_path(Path(DIR_CASES) / case_json_name(traj_id))

    def case_json_path_for_source(self, traj_id: int, path_source: PathSource | str) -> Path:
        return self.resolve_project_path(Path(DIR_CASES) / _path_source_dir(path_source) / case_json_name(traj_id))

    def bin_path(self, traj_id: int) -> Path:
        return self.resolve_project_path(Path(DIR_BIN) / bin_name(traj_id))

    def bin_path_for_source(self, traj_id: int, path_source: PathSource | str) -> Path:
        return self.resolve_project_path(Path(DIR_BIN) / _path_source_dir(path_source) / bin_name(traj_id))

    def final_bin_path(self, traj_id: int) -> Path:
        return self.resolve_project_path(Path(DIR_BIN) / DIR_FINAL / bin_name(traj_id))

    def portable_path(self, traj_id: int) -> Path:
        return self.resolve_project_path(Path(DIR_PORTABLE) / portable_name(traj_id))

    def portable_path_for_source(self, traj_id: int, path_source: PathSource | str) -> Path:
        return self.resolve_project_path(Path(DIR_PORTABLE) / _path_source_dir(path_source) / portable_name(traj_id))
    ...
    def ensure_directories(self) -> None:
        for path in (
            self.cases_dir,
            self.task_compiled_cases_dir,
            self.manual_free_cases_dir,
            self.bin_dir,
            self.task_compiled_bin_dir,
            self.manual_free_bin_dir,
            self.final_bin_dir,
            self.portable_dir,
            self.task_compiled_portable_dir,
            self.manual_free_portable_dir,
            self.reports_dir,
    ...
    def status(self) -> ProjectStatusReport:
        ...
        ready_cases = []
        for path in sorted(self.cases_dir.glob("P*.json")):
            try:
                case = load_case(path)
            except Exception:
                continue
    ...
    def status_for_case(self, traj_id: int) -> ProjectStatusReport:
        traj_id = validate_traj_id(traj_id)
        case_path = self.case_json_path(traj_id)
...
def _path_source_dir(path_source: PathSource | str) -> str:
    ...
    if source == PathSource.TASK_COMPILED:
        return DIR_TASK_COMPILED
    if source == PathSource.MANUAL_FREE:
        return DIR_MANUAL_FREE
    raise ProjectLayoutError(f"unsupported path_source: {path_source!r}")

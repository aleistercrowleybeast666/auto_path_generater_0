...
from hjmb_pathgen.codec.json_codec import load_leg_library, load_project, save_case, save_portable_case
...
class CaseOutputOptions:
    ...
    use_mode_layout: bool = False
    path_source: PathSource | None = None
...
def write_case_outputs(
    ...
    path_source = options.path_source or case.path_source
    case_path = _case_path(layout, case.traj_id, path_source, options) if options.write_case_json else None
    bin_path = _bin_path(layout, case.traj_id, path_source, options) if options.write_bin else None
    portable_path = _portable_path(layout, case.traj_id, path_source, options) if options.write_portable else None
...
def _case_path(layout: ProjectLayout, traj_id: int, path_source: PathSource, options: CaseOutputOptions) -> Path:
    if options.use_mode_layout:
        return layout.case_json_path_for_source(traj_id, path_source)
    return layout.case_json_path(traj_id)


def _bin_path(layout: ProjectLayout, traj_id: int, path_source: PathSource, options: CaseOutputOptions) -> Path:
    if options.final_bin:
        return layout.final_bin_path(traj_id)
    if options.use_mode_layout:
        return layout.bin_path_for_source(traj_id, path_source)
    return layout.bin_path(traj_id)


def _portable_path(layout: ProjectLayout, traj_id: int, path_source: PathSource, options: CaseOutputOptions) -> Path:
    if options.use_mode_layout:
        return layout.portable_path_for_source(traj_id, path_source)
    return layout.portable_path(traj_id)

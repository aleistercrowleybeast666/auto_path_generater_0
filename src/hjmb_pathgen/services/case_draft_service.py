...
from hjmb_pathgen.codec.json_codec import load_case, load_project, load_route_case_table, save_case
...
from hjmb_pathgen.services.task_compiler import CaseDraftBuildResult, build_case_draft
...
def generate_case_draft(
    ...
    *,
    ...
    existing_case = _load_existing_case(layout.case_json_path(traj_id))
    ...
    save_case(layout.case_json_path(traj_id), built.case)
...
def generate_all_case_drafts(layout: ProjectLayout) -> CaseDraftBatchResult:
    ...
    for row in sorted(table.cases, key=lambda item: item.traj_id):
        try:
            existing_case = _load_existing_case(layout.case_json_path(row.traj_id))
            built = build_case_draft(row, project, existing_case=existing_case)
            save_case(layout.case_json_path(row.traj_id), built.case)
...
def _case_result(layout: ProjectLayout, built: CaseDraftBuildResult) -> CaseDraftResult:
    ...
    return CaseDraftResult(
        ...
        case_path=layout.case_json_path(case.traj_id),
